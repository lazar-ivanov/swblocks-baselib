...
namespace std
{
    ...
    /*
     * libc++ (used by Clang 20.1.0+) requires explicit char_traits specialization
     * for non-standard character types like unsigned char.
     * This specialization enables std::basic_string<unsigned char> to work properly.
     */
    ...
    struct char_traits< unsigned char >
    {
        ...
        static const char_type* find( const char_type* p, std::size_t count, const char_type& ch )
        {
            if( count == 0 )
                return nullptr;
            return static_cast< const char_type* >( std::memchr( p, ch, count ) );
        }
