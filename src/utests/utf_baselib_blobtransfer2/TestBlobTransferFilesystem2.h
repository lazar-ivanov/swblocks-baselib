...
/*
 * Continuation of TestBlobTransferFilesystem.h, split so that no single test translation unit
 * exhausts a 32-bit compiler host - see notes/reviews/major/update_2026/test-module-split-plan.md
 *
 * The cut is at the lock boundary: every case in the first file takes the machine global test lock
 * and drives a blob server, while these drive the unpackager and packager units standalone. The one
 * exception is BlobTransfer_FilesPackagerInMemoryProxyTests near the end, which does take the lock.
 *
 * This file must be included after TestBlobTransferFilesystem.h - it is a continuation, not a
 * standalone header.
 */
