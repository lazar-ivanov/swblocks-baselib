...
#define UTF_TEST_MODULE utf_baselib_h2client
#include <utests/baselib/UtfMain.h>

/*
 * The HTTP/2 I/O shell: the connection task against the in-process test peer, cleartext and
 * TLS, the connection pool, GOAWAY and REFUSED_STREAM retry, timeouts and cancellation, and the
 * full-duplex stress suite on a multi-threaded I/O pool and again on a single-threaded one - see
 * notes/plans/http2-design.md 8.1 (D20)
 *
 * Sockets: loopback. The stress case is the one design 8.3 runs under ThreadSanitizer
 *
