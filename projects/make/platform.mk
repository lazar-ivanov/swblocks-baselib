...
ifeq (win, $(findstring win, $(OS)))
  ...
  ifeq (Darwin-15.,$(findstring Darwin-15.,$(UNAME_MERGED)))
    ...
    ifeq (Ubuntu,$(findstring Ubuntu,$(LSB_RELEASE_DIST_ID)))
        ...
        BL_PROP_PLAT := linux-$(OS)
        BL_PLAT_IS_UBUNTU := 1
    else ifeq (Pop,$(findstring Pop,$(LSB_RELEASE_DIST_ID)))
        #
        # Pop!_OS - version numbers mirror Ubuntu base versions directly
        # (Pop!_OS 24.04 is based on Ubuntu 24.04)
        #
        ifeq (24.04,$(findstring 24.04,$(LSB_RELEASE_VERSION)))
            ifneq ("$(wildcard $(DIST_ROOT_DEPS3)/toolchain-gcc/15.2.0)","")
                # This is devenv7; use ubuntu 24.04 binaries
                OS := ub24
            else ifneq ("$(wildcard $(DIST_ROOT_DEPS3)/toolchain-clang/20.1.0)","")
            ...
            $(error Unsupported Pop!_OS Version)
        endif
        DPKGDEB = fakeroot dpkg-deb --build
        UNAME_I := $(shell uname -i)
        ifeq ($(UNAME_I),i686)
        ...
        BL_PROP_PLAT := linux-$(OS)
        BL_PLAT_IS_UBUNTU := 1
    else ifeq (Elementary,$(findstring Elementary,$(LSB_RELEASE_DIST_ID)))
        #
        # Elementary OS - map to Ubuntu base via /etc/os-release UBUNTU_CODENAME
        #
        UPSTREAM_UBUNTU_CODENAME := $(shell grep UBUNTU_CODENAME /etc/os-release 2>/dev/null | cut -d= -f2)
        ifeq (noble,$(findstring noble,$(UPSTREAM_UBUNTU_CODENAME)))
            ifneq ("$(wildcard $(DIST_ROOT_DEPS3)/toolchain-gcc/15.2.0)","")
                # This is devenv7; use ubuntu 24.04 binaries
                OS := ub24
            else ifneq ("$(wildcard $(DIST_ROOT_DEPS3)/toolchain-clang/20.1.0)","")
            ...
            $(error Unsupported Elementary OS Ubuntu base version)
        endif
        DPKGDEB = fakeroot dpkg-deb --build
        UNAME_I := $(shell uname -i)
        ifeq ($(UNAME_I),i686)
