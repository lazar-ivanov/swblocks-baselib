...
| M-19 clang-cl global `-Wno-*` under `-WX` | **Recorded** | `windows-only-residual-findings-deferral.md` item 1 | on the next Windows build session |
...
| L-16 `LoadLibraryExW` flags, byte-widening | **Recorded** | `windows-only-residual-findings-deferral.md` item 4 | on the next Windows build session |
| L-17 `bl_tool` junction check fails open | **Fixed**: fail-closed `st_file_attributes` fallback, the `s3_manage.is_hostile_reparse` shape; the older plan record superseded | `scripts/bl_tool.py`, `directory-hash-tree-commitment-plan.md` | `TestReparseLinkDetection` (3 cases); Windows runtime confirmation recorded |
...
| L-20 include-note paths with spaces | **Fixed**: prefix parsing (`parse_include_note`) and make escaping (`escape_dependency_path`) in both identical wrappers | `scripts/cl.py`, `scripts/clang-cl.py` | `TestIncludeNoteParsing` (5 cases, one parses a generated `.d` with GNU make); Windows runtime confirmation recorded |
...
| L-33, L-34 Windows toolchain makefiles | **Recorded** | `windows-only-residual-findings-deferral.md` items 2-3 | on the next Windows build session |
...
| I-17 documentation drift, editor settings | **Fixed**: the JDK path lines and the OpenSSL CRT sentence in `scripts/devenv7/AGENTS.md`, the include-site sentence in `notes/clang_analysis_tools.md`; `settings/vscode/linux/a64` refreshed to the devenv7 layout with `${workspaceFolder}`; the x64, macOS and Windows settings and the two script sites are recorded | as named; `windows-only-residual-findings-deferral.md` item 7; deferral row 21 | `ls` of every path in the a64 file |
...
Not run: Windows and macOS (no host); OpenSSL 1.0.2 (no dist: L-10 and L-13 are desk-checked
against the 1.0.2 API surface); JDK 8; the Windows halves of L-17 and L-20 and the I-8 bound (see
`windows-only-residual-findings-deferral.md`).
