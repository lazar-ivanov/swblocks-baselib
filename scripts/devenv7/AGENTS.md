...
- **State each rule once.** Reference root `AGENTS.md` for shared rules (batch escaping, delayed expansion).
...
See root `AGENTS.md` for the full rules on special character escaping, delayed expansion (`!VAR!` inside control structures), line continuation in set commands, and the decision matrix.
